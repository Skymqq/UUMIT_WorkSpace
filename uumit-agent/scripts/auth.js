...
const { loadLocalPolicy, normalizePlatformId } = require('./package_common');
...
function registerCruiseSchedule(policy) {
  const dir = path.dirname(STATE_FILE);
  if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
  ...
  const effectivePolicy = policy || loadLocalPolicy(SKILL_DIR, process.env.UUMIT_AGENT_PLATFORM_TYPE || '');
  const scheduleRequest = buildCruiseScheduleRequest();
  const inboxScheduleRequest = buildInboxCruiseScheduleRequest(effectivePolicy);
  const applyScheduleRequest = buildApplyCruiseScheduleRequest(effectivePolicy);
  const deliverScheduleRequest = buildDeliverCruiseScheduleRequest(effectivePolicy);
...
async function main() {
  ...
  if (args.includes('--check')) {
    ...
    let walletData, cruiseData;
    try {
      const account = await getAccountInfo(creds.apiKey, creds.userId);
      walletData = account.wallet;
      cruiseData = account.cruise;
    } catch (e) {
      log(`API 查询失败: ${e.message}`);
      emitJson({ ok: false, error: 'credential_check_failed', retryable: true });
      return 1;
    }
    if (!walletData || !walletData.ut) {
      log('钱包信息无效');
      emitJson({ ok: false, error: 'credential_check_failed', retryable: true });
      return 1;
    }
    let agentPlatformType, policy, scheduleRequest, inboxScheduleRequest, applyScheduleRequest, deliverScheduleRequest, mcpRequest, runtimeConnectRequest, postAuth;
    try {
      agentPlatformType = resolveAgentPlatformType(args);
      policy = loadLocalPolicy(SKILL_DIR, agentPlatformType);
      scheduleRequest = registerCruiseSchedule();
      inboxScheduleRequest = buildInboxCruiseScheduleRequest(policy);
      applyScheduleRequest = buildApplyCruiseScheduleRequest(policy);
      deliverScheduleRequest = buildDeliverCruiseScheduleRequest(policy);
      mcpRequest = buildMcpRegistrationRequest(creds.apiKey, creds.userId, policy);
      runtimeConnectRequest = buildRuntimeConnectRequest();
      postAuth = buildPostAuthOnboarding(creds.apiKey, creds.userId, agentPlatformType);
    } catch (e) {
      log(`构建失败: ${e.message}`);
      log(e.stack);
      emitJson({ ok: false, error: e.message, stack: e.stack, retryable: false });
      return 1;
    }
    savePostAuthOnboarding(postAuth);
    const ut = walletData.ut || {};
    const profile = (cruiseData.profile && cruiseData.profile.profile) || {};
    emitJson({
      ok: true,
  ...
  // Check existing credentials first (unless --reset was used or --save-as to a different profile)
  const currentProfile = authCommon.getActiveProfileName();
  const skipCheck = args.some(a => ['--reset', '--force', '--reauth'].includes(a)) ||
    (saveAsProfile && saveAsProfile !== currentProfile);
  if (!skipCheck) {
  ...
  emitJson({
    ...
    wait_command: `node "${path.join(SKILL_DIR, 'scripts', 'auth.js')}" --wait ${authData.device_code}${saveAsProfile ? ' --save-as ' + saveAsProfile : ''}`,
    required_next_command: `node "${path.join(SKILL_DIR, 'scripts', 'auth.js')}" --wait ${authData.device_code}${saveAsProfile ? ' --save-as ' + saveAsProfile : ''}`,
