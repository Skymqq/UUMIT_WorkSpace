...
for page in range(1, 21):
    ...
    print(f"Page {page}: {len(items)} items, {sum(1 for t in items if t.get('status')=='open')} open (total={total})")
...
print()
print("First 20 open tasks:")
for t in open_tasks[:20]:
    uid = t.get("user_id", "")[:12]
    status = t.get("my_application_status")
    bounty = t.get("bounty_amount", "?")
    cat = t.get("category", "?")
    title = t.get("title", "")[:45]
    print(f"  user={uid} | {bounty}UT | {cat} | my_app={status} | {title}")
